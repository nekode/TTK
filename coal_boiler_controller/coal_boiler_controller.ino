...
int temp_val = 45; //Установленная температура
...
unsigned long period_millis = 0; //переменная времени для периодического обновления данных
unsigned long seconds_millis = 0; //переменная времени для периодического обновления данных
unsigned long smoke_overtemp_millis = 0; // переменная для интервала проверки перегрева дымохода
unsigned long temp_logging_millis = 0;
unsigned long temp_logging__47_millis = 0;
int period = 3500; //период обновления данных термометров и движения сервы заслонки (миллисекунды)
double Setpoint, Input, Output; // Переменные для работы ПИД алгоритма.
//double Kp=135, Ki=0, Kd=0; // Переменные для работы ПИД алгоритма. //Kp колебаний 135, период 218 с.
double Kp=45, Ki=0, Kd=0; // Переменные для работы ПИД алгоритма.
PID myPID(&Input, &Output, &Setpoint, Kp, Ki, Kd, DIRECT);//создаем ПИД-регулятор
int PID_interval = 30000; // Интервал рассчёта ПИД коэффициента в миллисекундах
int zaslonka_val; // положение заслонки
int zaslonka_min = 33; // Заслонка закрыта
int zaslonka_max = 165; // Заслонка открыта
int zaslonka_val_old = zaslonka_min; // переменная хранения предыдущего значения заслонки
int zaslonka_temp = zaslonka_min; // переменная для плавного изменения положения заслонки
int cooling_counter = 0; // Переменная отсчёта остывания (сек)
int cooling_limit_1 = 1800; // Переменная времени детектирования остывания (сек)
int cooling_limit_2 = 5400; // Переменная окончания индикации остывания (сек)
...
const int knopka_rezhim = 12; // кнопка выбора режима				   
const int knopka = 5; // кнопка энкодера
const int POMPA = 6; // пин насоса (управление низким уровнем)
const int BEEPER = 4; // пин пьезобузера
const int RELE = 7; // резерв
const int SERVO = 3; // пин управления сервой
const int thermoDO = 8; //подключение max6675
const int thermoCS = 9; //подключение max6675
const int thermoCLK = 10; //подключение max6675
int tempthermocouple_value = 0; // Переменная температуры дымохода
int tempthermocouple_value_limit = 115; // Значение ограничения температуры дымохода
int tempthermocouple_value_differential = 7; // Значение гистерезиса ограничения температуры дымохода

MAX6675 thermocouple(thermoCLK, thermoCS, thermoDO);

int pos = 0; // для хранения текущего положения энкодера																							
...
void setup()
{
  ...
  myPID.SetOutputLimits(zaslonka_min, zaslonka_max);//устанавливаем границы выходного сигнала для ПИД-регулятора
  myPID.SetMode(AUTOMATIC); // Включаем ПИД-регулятор
  myPID.SetSampleTime(PID_interval); // Задает частоту расчета выходного сигнала, по умолчанию 200 мили секунд
  Serial.begin(9600);
...
//  EEPROM.get(4, zaslonka_val);  //Чтение из ЕЕПРОМ 
  EEPROM.get(6, rezis);   //Чтение из ЕЕПРОМ 
...
void loop()
{
  ...
  if ((millis()-period_millis)>period) // Считываем показания температуры с датчиков с интервалом "period"
	...
	if (temp_carbon != temp_logging)
	{
    Serial.print(temp_carbon);	
    Serial.print(" > ");
	Serial.print(millis() - temp_logging_millis);
	temp_logging_millis = millis();
	if (temp_carbon == 47.0) 
		{
		Serial.print("    >>>>     ");
		Serial.print(millis() - temp_logging__47_millis);		
		temp_logging__47_millis = temp_logging_millis;
		}    
		Serial.println ("");
	temp_logging = temp_carbon;
...
if (tempthermocouple_value > tempthermocouple_value_limit)
	{
    if ((millis() - smoke_overtemp_millis)>7000) // проверяем, не пора ли проверить перегрев дымохода	
		{
		smoke_overtemp_millis = millis();
		if (!smoke_temp_max_state)  // если режим ограничения температуры дымохода ещё не включён
		{
		  smoke_temp_max_state = 1; // взводим флаг режима ограничения температуры дымохода
		  myPID.SetMode(MANUAL); // Выключаем ПИД-регулятор
		}
		zaslonka.attach(SERVO); // Иницилизация серво
		int zaslonka_temp = zaslonka.read(); // Считываем с сервы последнее записаное значение
		if (zaslonka_temp > (zaslonka_min + 2)) 
			{			
			zaslonka_temp = (zaslonka_temp - 3);
...
//  else if ((tempthermocouple_value < tempthermocouple_value_limit) && (tempthermocouple_value > (tempthermocouple_value_limit - tempthermocouple_value_differential)) && ((smoke_temp_max_state)))
    else if (tempthermocouple_value < (tempthermocouple_value_limit - tempthermocouple_value_differential) && smoke_temp_max_state) // если температура дымохода нормализовалась
    {
	  smoke_temp_max_state = 0; // опускаем флаг перегрева дымохода
      myPID.SetMode(AUTOMATIC); // Включаем ПИД-регулятор
      myPID.SetSampleTime(PID_interval); // Задает частоту расчета выходного сигнала, по умолчанию 200 мили секунд
    }
...
void zaslonka_move() 
{
  ...
  Serial.println("*");
		delay(20);
		if (!Output==0.00) // Если алгоритм ПИД выдал значение, отличное от нуля (истёк очередной интервал рассчёта ПИД, заданный в дефайнах или изменились входные данные)
		{		
			zaslonka.attach(SERVO); // Иницилизация серво
			int zaslonka_temp = zaslonka.read(); // Считываем с сервы последнее записаное значение
//			zaslonka_temp = zaslonka_val_old;
			zaslonka_val=int(Output);	// Присваиваем значение, посчитанное алгоритмом PID переменной, приведя тип от флоат к инт
...
void set_temp()
{
static unsigned long timeout_mode_set_temp = 0;
...
void menu()
{
static unsigned long timeout_mode_menu = 0;
int newPos = encoder.getPosition();
    if (((pos - 2) >= newPos) || ((pos + 2) <= newPos))
  {
    if (newPos > pos)
